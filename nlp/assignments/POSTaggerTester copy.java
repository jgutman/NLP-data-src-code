...
public class POSTaggerTester {
	...
	static class TrigramHMMScorer extends MostFrequentTagScorer {
		...
		public Counter<String> getLogScoreCounter(
			...
			Set<String> possibleTags = tagsToWords.keySet();
			
			Counter<String> tagCounter = unknownWordTags;
			if (knownWord) {
				Counter<String> HMMscore = new Counter<String>();
				for (String tag : possibleTags) {
					double transition = trigramCounter.getCount(previousBigram, tag);
					double emission = tagsToWords.getCount(tag, word);
					double score = transition * emission;
					HMMscore.setCount(tag, score);
				}
				tagCounter = HMMscore;
			}
			tagCounter = Counters.normalize(tagCounter);
				
			//What is allowedFollowingTags?
	...
	public static void main(String[] args) throws Exception {
		...
		LocalTrigramScorer localTrigramScorer = new TrigramHMMScorer(false);
