...
public class WordAlignmentTester {
  ...
  static class IBMmodel1WordAligner implements WordAligner {
			  ...
			  for (SentencePair sentencePair : trainingSentencePairs) {
			      ...
			      for (int frenchPosition=0; frenchPosition < numFrenchWords; frenchPosition++) {
			    	  ...
			    	  for (int englishPosition=nullPosition; englishPosition < numEnglishWords; englishPosition++) {
			    		  ...
			    		  if (!initializedT.contains(fe)) { // check if t(f|e) has been initialized, otherwise set to uniform
			      ...
			      for (int frenchPosition=0; frenchPosition < numFrenchWords; frenchPosition++) {
			    	  ...
			    	  for (int englishPosition=nullPosition; englishPosition < numEnglishWords; englishPosition++) {
			    		  ...
			    		  double tq = translations.getCount(f, e) * alignments.getCount(englishPosition);
			    		  double delta = tq  / normalizationConstant.getCount(f);
	  ...
	  public IBMmodel1WordAligner(List<SentencePair> data, int maxIterations) {
		  this.trainingSentencePairs = data;
  ...
  static class IBM2BucketsWordAligner extends IBMmodel2WordAligner implements WordAligner {
	  //List<SentencePair> trainingSentencePairs; // inherited
	  //CounterMap<String,String> translationProb; // inherited
	  //static final String NULL = "<NULL>"; // inherited
	  Counter<Integer> distortionBuckets = new CounterMap<Integer>();
	  static final int minDistortion = -5;
	  static final int maxDistortion = 5;
	  static final int numBuckets = 1 + (maxDistortion - minDistortion);
	  static final int nullBucket = minDistortion - 1;
	  
	  private void reallocateNullMass (Counter<Integer> probability, double kNullProb, int nullIndex) {
		  probability.scale(1.0 - kNullProb);
		  probability.setCount(nullIndex, kNullProb);
	  }
	  
	  privaate void reallocateNullMass (Counter<Integer> probability, double kNullProb) {
		  reallocateNullMass(probability, kNullProb, nullBucket);
	  }
	  
	  private int computeDistortionBucket (int frenchPosition, int englishPosition,
			  int numFrenchWords, int numEnglishWords) {

		  int hDiscretized;
		  if (englishPosition == nullPosition) {
			  hDiscretized = nullBucket;
		  } else {
			  double h = (englishPosition+1) - ((frenchPosition+1) * numEnglishWords / numFrenchWords);
			  hDiscretized = Math.round(h);
			  hDiscretized = Math.max(hDiscretized, minDistortion);
			  hDiscretized = Math.min(hDiscretized, maxDistortion);
		  }
		  return hDiscretized;
	  }
  }
  
  static class IBMmodel2WordAligner extends IBMmodel1WordAligner implements WordAligner {
	  // List<SentencePair> trainingSentencePairs; // inherited
	  // CounterMap<String,String> translationProb; // inherited
	  CounterMap<Triple,Integer> alignmentProb;
	  boolean initializeUniform;
	  boolean useDistortionNormalized;
	  // static final String NULL = "<NULL>"; // inherited
	  // static final int nullPosition = -1; // inherited
	  
	  // hyperparameters of the model, ideally we would like to learn these values from data
	  final double alpha = 1.0; // this controls how strong the preference for diagonal is in the exponential model
	  ...
	  public Alignment alignSentencePair (SentencePair sentencePair) {
		  ...
		  // So we take the argMax of t(f|e) * q(j|i,l,m) to get the englishMaxPosition
		  for (int frenchPosition = 0; frenchPosition < numFrenchWords; frenchPosition++) {
			  String f = frenchWords.get(frenchPosition);
			  Triple ilm = new Triple (frenchPosition, numEnglishWords, numFrenchWords);
			  ...
			  double maxProbability = translationProb.getCount(f, NULL) * alignmentProb.getCount(ilm, englishMaxPosition);
			  for (int englishPosition = 0; englishPosition < numEnglishWords; englishPosition++) {
				  String e = englishWords.get(englishPosition);
				  double probability = translationProb.getCount(f, e) * alignmentProb.getCount(ilm, englishPosition);
				  if (probability > maxProbability) {
					  englishMaxPosition = englishPosition;
					  maxProbability = probability;
				  }
	  ...
	  private ParameterEstimate trainEM (int maxIterations) {
		  CounterMap<String,String> translations = new CounterMap<String,String>();
		  CounterMap<Triple,Integer> alignments = new CounterMap<Triple,Integer>();
		  Set<String> frenchVocab = new HashSet<String>();
		  
		  // initialize the parameter estimates
		  Set<Pair<Integer,Triple>> initializedQ = new HashSet<Pair<Integer,Triple>>();
		  Set<Pair<String,String>> initializedT = new HashSet<Pair<String,String>>();
		  double initialCount = 1.0;
		  CounterMap<String,String> initialTranslations = new CounterMap<String,String>();
		
		  // This only deals with initialization of t(f|e) parameters: initialize uniform or from IBM Model 1?
		  // Note: we can do a similar thing with IBM Model 1 flag, initialize as uniform or from heuristic model
		  // IBM Model 1 is convex so this is only necessary if we do not iterate to convergence
		  if (initializeUniform) {
			  ...
			  // We need to initialize translationProb.getCount(f,e) uniformly
			  // for all e in {E + null} : t(f|e) summed over all f in {F} = 1
			  initialCount = 1.0 / frenchVocab.size();
		  }
		  else {
			  // if not initializing with uniform probabilities, then we initialize t with 
			  // estimates obtained from EM using IBM model 1
			  
			  IBMmodel1WordAligner initialAligner = new IBMmodel1WordAligner(trainingSentencePairs, maxIterations/2);
			  initialTranslations = initialAligner.translationProb;
		  }
		  
		  //useDistortionBuckets learns the distribution of alignments over bucket distance from the data
		  //therefore, only initialized once
		  //contrast with useDistortionNormalized which we treat as a constant function of the distortion distance
		  int numBuckets = 1 + (maxDistortion - minDistortion);
		  if (useDistortionBuckets) {
			  for (int bucket=minDistortion; bucket <= maxDistortion; bucket++) {
				  distortionBuckets.setCount(bucket, 1.0 / numBuckets);
				  reallocateNullMass (distortionBuckets, reservedNullProb, minDistortion-1);
			  }
		  }
		  ...
		  for (int s=0; s < maxIterations; s++) {
			  ...
			  CounterMap<String,String> counts = new CounterMap<String,String>(); // set count(f|e) to 0 for all e,f
			  Counter<String> totalEnglish = new Counter<String>(); // set total(e) to 0 for all e
			  CounterMap<Triple,Integer> alignmentCounts = new CounterMap<Triple,Integer>();
			  Counter<Triple> totalAlignments = new Counter<Triple>();
			  ...
			  for (SentencePair sentencePair : trainingSentencePairs) {
				  ...
				  // Initialize t and q parameters and compute normalization constant to get delta
				  for (int frenchPosition=0; frenchPosition < numFrenchWords; frenchPosition++) {
					  String f = frenchWords.get(frenchPosition);
					  String e = NULL;
					  Triple ilm = new Triple(frenchPosition, numEnglishWords, numFrenchWords);
					  if (useDistortionNormalized) { // recalculuate distribution given new i,l,m for all j
						  distortionBuckets = computeNormalizedDistortion (frenchPosition,
								  numFrenchWords, numEnglishWords, alpha);
						  reallocateNullMass (distortionBuckets, reservedNullProb, -1);
					  }
					  for (int englishPosition=-1; englishPosition < numEnglishWords; englishPosition++) {
						  if (englishPosition > -1) {
							  e = englishWords.get(englishPosition);
						  } 
						  Pair<String,String> fe = Pair.makePair(f,e);
						  if (!initializedT.contains(fe)) { // check if t(f|e) has been initialized
							  if (initializeUniform)
								  translations.setCount(f, e, initialCount);
							  else
								  translations.setCount(f, e, initialTranslations.getCount(f,e));
							  initializedT.add(fe);
						  }
						  
						  if (useDistortionBuckets){ //re-initialize alignments from distortionBuckets every time
							  int bucket = computeDistortionBucket (frenchPosition, englishPosition,
									  numFrenchWords, numEnglishWords, minDistortion, maxDistortion);
							  alignments.setCount(ilm, englishPosition, distortionBuckets.getCount(bucket));
						  } else {
							  Pair<Integer,Triple> jilm = Pair.makePair(englishPosition, ilm);
							  if (!initializedQ.contains(jilm)) { // check if q(j|i,l,m) has been initialized
								  if (useDistortionNormalized ) {
									  alignments.setCount(ilm, englishPosition, distortionBuckets.getCount(englishPosition));
								  } else {
									  alignments.setCount(ilm, englishPosition, getInitialQ(ilm));
								  }
								  initializedQ.add(jilm);
							  } 
						  }
						  
						  // compute normalization constant over all English words, including NULL
						  double tq = translations.getCount(f, e) * alignments.getCount(ilm, englishPosition);
			    		  normalizationConstant.incrementCount(f, tq);  
					  }
				  }
				  
				  // Now we have delta(k, i, j) = translations.getCount(f,e) * alignments.getCount(ilm,j) /
				  // normalizationConstant.getCount(f) for all f,e, and i,j in training sentencePair k 
				  ...
				  for (int frenchPosition=0; frenchPosition < numFrenchWords; frenchPosition++) {
					  ...
					  for (int englishPosition=-1; englishPosition < numEnglishWords; englishPosition++) {
						  ...
						  double delta = translations.getCount(f,e) * alignments.getCount(ilm, englishPosition) / 
								  normalizationConstant.getCount(f);
						  ...
						  if (useDistortionBuckets) {
							  Triple constant = new Triple (-1, -1, -1);
							  int bucket = computeDistortionBucket (frenchPosition, englishPosition,
									  numFrenchWords, numEnglishWords, minDistortion, maxDistortion);
							  alignmentCounts.incrementCount(constant, bucket, delta);
							  totalAlignments.incrementCount(constant, delta);
						  } else if (!useDistortionNormalized){
							  alignmentCounts.incrementCount(ilm, englishPosition, delta);
							  totalAlignments.incrementCount(ilm, delta);
						  }
					  }
				  }
			  } // All sentences trained, end of count estimation (E-step)
			  ...
			  // Update parameter estimates from estimated expected counts (M-step)
			  CounterMap<String,String> t = new CounterMap<String,String>();
			  CounterMap<Triple,Integer> q = new CounterMap<Triple,Integer>();
			  ...
			  if (useDistortionBuckets) {
				  Triple constant = new Triple(-1, -1, -1);
				  double normalizer = totalAlignments.getCount(constant);
				  for (int bucket : alignmentCounts.getCounter(constant)).keySet()) {
					  double expectedCount = alignmentCounts.getCount(constant, bucket);
					  distortionBuckets.setCount(bucket, expectedCount / normalizer;)
				  }  
			  } else if (!useDistortionNormalized) {
				  for (Triple ilm : alignmentCounts.keySet()) {
					  double normalizer = totalAlignments.getCount(ilm);
					  for (int j : (alignmentCounts.getCounter(ilm)).keySet()) {
						  double expectedCount = alignmentCounts.getCount(ilm, j);
						  q.setCount(ilm, j, expectedCount / normalizer);
					  }
				  }
				  alignments = q;
			  } 
		  ...
		  ParameterEstimate parameters = new ParameterEstimate(translations, alignments);
		  return parameters;
	  }
	  
	  private double getInitialQ(Triple ilm) {
		  int possibleAlignments = 1 + ilm.L();
		  double uniformProb = 1.0 / possibleAlignments;
		  return uniformProb;
	  }
	  
	  private Counter<Integer> computeNormalizedDistortion (int frenchPosition,
			  int numFrenchWords, int numEnglishWords, double alpha) {
		  Counter<Integer> distortionProb = new Counter<Integer>();
		  
		  for (int englishPosition=0; englishPosition < numEnglishWords; englishPosition++) {
			  double h = (englishPosition+1) - ((frenchPosition+1) * numEnglishWords / numFrenchWords);
			  h = -1.0 * alpha * h;
			  distortionProb.setCount(englishPosition, Math.exp(h));
		  }
		  distortionProb.normalize();
		  return distortionProb;
	  }
	  ...
	  private int computeDistortionBucket (int frenchPosition, int englishPosition,
			  int numFrenchWords, int numEnglishWords,
			  int minDistortion, int maxDistortion) {
		  
		  final int nullBucket = minDistortion - 1;
		  int hDiscretized;
	  ...
	  public IBMmodel2WordAligner(List<SentencePair> data, int maxIterations, 
			  boolean initializeUniform, boolean useDistortionNormalized) {
		  this.trainingSentencePairs = data;
		  this.initializeUniform = initializeUniform;
		  this.useDistortionNormalized = useDistortionNormalized;
		  ParameterEstimate parameters = trainEM(maxIterations);
		  translationProb = parameters.t;
		  alignmentProb = parameters.q;
	  }
  }
  ...
  public static void main(String[] args) throws IOException {
    ...
    // TODO : build other alignment models
    	...
    	wordAligner = new IBMmodel1WordAligner(trainingSentencePairs, maxIterations);
    }
    else if (model.equalsIgnoreCase("ibm2") || model.equalsIgnoreCase("ibmModel2")) {
    	wordAligner = new IBMmodel2WordAligner(trainingSentencePairs, maxIterations, initialize, false);
    }
