...
public class WordAlignmentTester {
  ...
  static class IBMmodel2WordAligner  implements WordAligner {
	  ...
	  static final double minimumThreshold = 1e-6;
	  ...
	  public Alignment alignSentencePair (SentencePair sentencePair) {
		  ...
		  for (int frenchPosition = 0; frenchPosition < numFrenchWords; frenchPosition++) {
			  String f = frenchWords.get(frenchPosition);
			  Triple ilm = new Triple (frenchPosition, numEnglishWords, numFrenchWords);
			  if (!alignmentProb.containsKey(ilm)) {
				  Counter<Integer> distortions;
				  if (useDistortionNormalized) {
					  distortions = getInitialQ(ilm, alpha);
				  }
				  else {
					  distortions = getInitialQ(ilm);
				  }
				  for (int englishPosition = nullPosition; englishPosition < numEnglishWords; englishPosition++) {
					  alignmentProb.setCount(ilm, englishPosition, distortions.getCount(englishPosition));
				  }
			  }
		  ...
		  for (int s=0; s < maxIterations; s++) {
			  ...
			  for (SentencePair sentencePair : trainingSentencePairs) {
				  ...
				  for (int frenchPosition=0; frenchPosition < numFrenchWords; frenchPosition++) {
					  ...
					  String e = NULL;
					  Triple ilm = new Triple(frenchPosition, numEnglishWords, numFrenchWords);
					  if (!alignments.containsKey(ilm)) { // check if q(j|i,l,m) has been initialized 
						  if (useDistortionNormalized) {
							  distortions = getInitialQ(ilm, alpha); // exponential distance weighted alignment prob, NULL reserved
						  }
						  else {
							  distortions = getInitialQ(ilm); // uniform initial alignment prob, with NULL reserved
						  }
						  for (int englishPosition = nullPosition; englishPosition < numEnglishWords; englishPosition++) {
							  alignments.setCount(ilm, englishPosition, distortions.getCount(englishPosition));
						  }
					  }
					  
					  for (int englishPosition = nullPosition; englishPosition < numEnglishWords; englishPosition++) {
						  if (englishPosition > nullPosition) {
							  e = englishWords.get(englishPosition); // initialized to NULL if nullPosition
						  } 
						  
						  if (!(translations.getCounter(f)).containsKey(e)) { // check if t(f|e) has been initialized
						  ...
						  // compute normalization constant over all English words, including NULL
						  if (translations.getCount(f, e) < minimumThreshold) 
							  translations.setCount(f, e, minimumThreshold);
						  if (alignments.getCount(ilm, englishPosition) < minimumThreshold)
							  alignments.setCount(ilm, englishPosition, minimumThreshold);
						  
						  double delta = translations.getCount(f, e) * alignments.getCount(ilm, englishPosition);
						  System.out.println("iteration "+(s+1)+ " delta "+delta);
						  normalizationConstant.incrementCount(f, delta);  
					  }
					  System.out.println("normalization "+ normalizationConstant);
				  }
				  ...
				  for (int frenchPosition=0; frenchPosition < numFrenchWords; frenchPosition++) {
					  ...
					  for (int englishPosition = nullPosition; englishPosition < numEnglishWords; englishPosition++) {
						  ...
						  if (normalizationConstant.getCount(f) > minimumThreshold) {
							  delta = translations.getCount(f,e) * alignments.getCount(ilm, englishPosition) /
									  normalizationConstant.getCount(f);
							  // increment expected counts using delta
							  counts.incrementCount(e, f, delta);
							  totalEnglish.incrementCount(e, delta);
							  if (!useDistortionNormalized) { 
								  // we don't need to update alignments for useDistortionNormalized unless we want to train alpha
								  // but training alpha with EM probably cannot be solved analytically, need gradient descent
			  ...
			  for (String e : counts.keySet()) {
				  ...
				  for (String f : (counts.getCounter(e)).keySet()) {
					  double expectedCount = counts.getCount(e, f);
					  t.setCount(f, e, Math.max(expectedCount / normalizer, minimumThreshold));
				  }
			  }
			  translations = t;
			  ...
			  if (!useDistortionNormalized) {
				  for (Triple ilm : alignmentCounts.keySet()) {
					  ...
					  for (int j : (alignmentCounts.getCounter(ilm)).keySet()) {
						  ...
						  q.setCount(ilm, j, Math.max(expectedCount / normalizer, minimumThreshold));
					  }
				  }
				  alignments = q;
			  }
	  ...
	  public IBMmodel2WordAligner(List<SentencePair> data, int maxIterations, 
		  ...
		  this(data, maxIterations, initializeUniform, useDistortionNormalized, 0.2);
	  }
	  
	  public IBMmodel2WordAligner(List<SentencePair> data, int maxIterations, 
			  boolean initializeUniform, boolean useDistortionNormalized, double reservedNull) {
		  this.reservedNullProb = reservedNull;
		  this.trainingSentencePairs = data;
  ...
  public static void main(String[] args) throws IOException {
    ...
    boolean initializeUniform = true;
    ...
    // TODO : build other alignment models
    	...
    	wordAligner = new IBMmodel2WordAligner(trainingSentencePairs, maxIterations, initializeUniform,
    			distortion, reservedNull);
    }
